...
| --- | --- | --- | --- | --- |
| M4 Nightly run 1/5 | root | Candidate `d709aa5e` on remote ref `codex/p4-nightly-m4-measurement-20260828`; push exact ref, then `gh workflow run nightly-verification.yml --ref codex/p4-nightly-m4-measurement-20260828`; no other owner may dispatch or cancel this lane. Stop after any deterministic code/topology failure; continue through infrastructure-only failure only with explicit classification. | `.runtime/reports/generated/m4-nightly-five-run/` plus GitHub run artifacts | planned |
