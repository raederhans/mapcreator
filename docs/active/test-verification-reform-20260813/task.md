...
- [x] P3.5 Political Partial-Repaint / Worker Engine: candidate commit, findings-first review, focused exact-selector proof, supervisor integration checkpoint.
- [x] Run the final bounded P3 acceptance ladder, close task records, push normally, and verify local/remote SHA equality. Production deployment and force-push remain unauthorized.
...
## Runtime Architecture Reset v1 — Verification tiers

- [x] Add canonical `verify:commit`: portfolio/source authority, import-boundary check, selector route check, then one batched Node control-plane contract and (when present) an adaptive child-safe run limited to non-control-plane changed files.
- [x] Retain the stricter local `verify:edit` entrypoint and preserve the distinct `verify:pr`, `verify:core` (main), Nightly, and release tiers.
- [x] Keep Pages build/dist drift, full-browser execution, and release-only gates outside `verify:commit`; selected Node tests continue through the canonical adaptive execution planner, which batches compatible leaves and keeps Python process-isolated.
- [x] Run the final changed-file `verify:commit` observation: 342 scripts, import graph for 51 specs, and all 398 selector routes pass; its fixed control-plane batch passes 190 tests. This slice changed only declared control-plane files, so no adaptive product selection was needed. Full Core, PR, Nightly, release, Pages, browser, and performance gates remain separate.
- [x] Harden follow-up ingress: derive the control-plane source set from canonical `infra:local-verification-closure`; retain both paths for porcelain rename/copy records; and accept only repeated explicit `--changed-file <path>` CLI inputs, falling back to working-tree discovery only when absent. The explicit child-safe `js/core/feature_identity.js` CLI observation executes one adaptive command successfully without widening to browser, Pages, or performance gates.
- [x] Preserve shared-source overlap: a control-closure source also referenced by any other canonical record enters both the fixed control batch and adaptive edit. The explicit `package.json` probe runs its 191-test control batch, then correctly fails closed with three existing adaptive route/command gaps rather than silently reducing product coverage.
