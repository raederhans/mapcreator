...
The remaining actionable backlog is limited to seven source-harder or hierarchy-mixed seas: Makassar Strait, Ross Sea, Arafura Sea, Timor Sea, Java Sea, Greenland Sea, and Bering Sea.
...
- [ ] Batch 3: design source-backed child geometry for the remaining seven hierarchy-mixed or alias-missing seas.

## Validation gates

- [x] Run `python tools\audit_tno_water_family_refinement.py`.
- [x] Run targeted water-family unittest.
- [x] Run `python tools\validate_tno_water_geometries.py --scenario-dir data\scenarios\tno_1962 --report-path .runtime\reports\generated\ocean_refinement_completion_geometry.json`.
- [x] Run `python tools\patch_tno_1962_bundle.py --changed-domain water --checkpoint-dir .runtime\build\scenario\tno_1962\ocean_refinement_completion`.
- [ ] Run `git diff --check`.
