...
| 2026-08-15 | Gate 0–4 integrated work 与 A 前结构修复已进入 main lineage；`c9baa13c` 固定为 pre-A baseline | Gate 1 的 `index.html`、`css/style.css`、`js/ui/toolbar.js` 变更作为 inherited upstream content；pure artifact model 与 detached scalar repair 将 toolbar 从 3186 行收敛到 3095 行，并消除 imported helper 的 full-state alias edge；A-specific delta 对三条共享路径保持零变化。 |
...
| 2026-08-15 | Export artifact deterministic projections 迁入纯 model | `c9baa13c` 保持 DOM、runtime mutation、bake cache 与下载副作用在既有 owners；helper 输入为 detached scalar；toolbar structural contract 保持 54/54 green。 |
