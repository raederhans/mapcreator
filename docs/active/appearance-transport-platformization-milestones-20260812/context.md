...
| A final review-fix P4.3 generator | A task delegated owner，主监督授权 | 在本 coordination commit 的 clean exact HEAD 执行 canonical generator；输出 `tools/state_writer_policy.json` | pending；单一 generator lane，成功后由主监督读取 schema/latestPhase/checkpoint 与 diagnostics，再提交 policy。 |
...
提交本次 coordination update，在 clean exact HEAD 授权 A 执行唯一 P4.3 generator；主监督随后提交 checkpoint并执行 exact routes、core、Pages、browser、standard perf 与最终 verdict。
