...
13. The central contract is source-grounded by the two production catalogs, the entry host, transport normalization owner, and UI-state normalization source. Its reviewed dependency snapshot verifies path existence and static import reachability; the former mirrored `EXPECTED_INVENTORY` oracle was removed.
