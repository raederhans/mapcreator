...
- Integration branch: none active; latest closeout from `codex/strategic-overlay-cleanup-followups`
...
1. Preserve historical recovery rows below; they are references only.
2. Re-check recovery commit hashes before recreating any historical worktree.
3. Treat `origin/codex/tno-toponym-zh-audit` as a separate remote-only branch review, outside this renderer-owner pass.
