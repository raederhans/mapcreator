...
- current task phase: P2.2a browser acceptance is green; rerun01 remains immutable after the WMI watcher permission failure; the normal-user containment path now uses a tracked Windows Job Object helper with compile-once preparation, fail-closed cleanup evidence, source/binary identity, and focused Windows integration coverage; a fresh governed rerun remains pending and P2.2b entry stays closed
- current acceptance test-contract commit: `427e68398a67586ef4a330b5304dfde567da917e`
- current worktree state: isolated P2 branch contains the committed analyzer repair through `557cdf34204b30d07f6c260841edc70531527b38` plus the uncommitted Job Object containment TDD slice; rerun01 temporary worktrees and junctions are removed; parent WIP remains untouched
...
The WMI watcher path has been replaced in the current uncommitted TDD slice. `tools/perf/williams_crossover_windows_job_runner.cs` is a tracked Windows helper. `tools/perf/williams_crossover_windows_runtime.mjs` compiles it once before the first pre-telemetry window, performs a short capability probe, reuses the binary for every block, and deletes the temporary compiled artifact after the experiment. `runLoggedCommand()` now delegates to this helper; the measurement path contains no WMI process-start watcher, system process polling, or `taskkill` cleanup.
