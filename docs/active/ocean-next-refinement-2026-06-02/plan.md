...
- [x] Add `tno_rye_bay` and `tno_belfast_lough` as source-backed child waters.
- [x] Publish checked-in water runtime, chunks, startup bundles, manifest, audit, and provenance with 132 water features.
- [x] Run final review and fix water summary drift.
...
- `tno_strait_of_dover` and `tno_north_channel` now have verified SeaVoX child records published as `tno_rye_bay` and `tno_belfast_lough`, with parent geometries cut to zero child overlap.
...
- The publish blocker for water-only checked-in edits is reduced: `water_runtime_from_scenario` now passes prechunk checkpoint validation with 132 water features, and the normal `--changed-domain water` path executes `water_runtime_from_scenario -> write_bundle -> chunk_assets`. The older full generator still carries unrelated global ocean topology failures and should be treated as a separate repair lane.
