...
## 2026-05-20 startup/apply perf gate repair

- Worktree `C:/Users/raede/Desktop/dev/mapcreator-perf-startup-g001` continues the same G001 story from pushed `origin/main@b923b22`.
- Fresh `npm run perf:gate` reproduced the blocker: TNO failed `applyScenarioBundleMs`; HOI4 failed `totalStartupMs`, `scenarioAppliedMs`, and `applyScenarioBundleMs`.
- Raw run evidence showed `applyScenarioBundleMs` was mostly waiting on `preloadScenarioCoarseChunks()` inside `ensureChunkedScenarioFirstFrameReady()`. The startup bundle already exposes a shell/core political frame, so startup apply does not need to await full coarse chunk hydration.
- Fix: `startup_scenario_boot.js` now passes `deferChunkPrewarm: true` only for the normal startup-bundle apply. Legacy bootstrap recovery keeps the conservative synchronous prewarm path. `runPostScenarioApplyEffects()` keeps manual apply behavior synchronous by default, but startup apply schedules `scenario-apply` chunk refresh and records `chunkedFirstFramePrewarm.awaited=false` / `chunkPrewarmDeferred=true` / `coarsePrewarmDeferredAt`.
