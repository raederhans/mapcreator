...
| Render/export | completed | map renderer 通过 `special_zone_layers_render_owner` 渲染 memberFeatureIds，包含 pattern defs、zoom transform 与 merged outline cache；export workbench 增加 `special-zones` 独立 SVG layer |
...
| Tests/i18n/review | completed | node --check、special-zone unit、176 个 Python targeted tests、i18n audit、scenario contracts 已通过；最终 reviewer blocker、render owner 收口与 legacy freehand 主路径退休已处理 |
