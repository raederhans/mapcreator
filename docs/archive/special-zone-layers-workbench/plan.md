...
## 执行阶段
- [x] 1. 建立 schema/state/manager/preset/pattern 基础模块。
- [x] 2. 接入 file manager、history manager、interaction funnel。
- [x] 3. 接入场景 asset loader 与 manifest 字段。
- [x] 4. 建立 render owner，并用最小 facade 接入 renderer。
- [x] 5. 接入 export layer。
- [x] 6. 建立 workbench controller 与 toolbar 入口。
- [x] 7. 接入地图编辑 tool 与批量操作。
- [x] 8. 接入场景写盘 endpoint。
- [x] 9. 移除旧 freehand/manual/scenario override 主路径。
- [x] 10. 补齐 i18n、a11y、状态提示。
- [x] 11. 跑 targeted tests 与最终 review。
