...
## 2026-05-06 comprehensive follow-up: current investigation
- User reports remaining non-interactive Mediterranean sea, strange water colors, renewed east Mediterranean issues, ring-Mediterranean land still non-interactive, some land colored as ocean, plus zoom stalls/black frames and disappearing political colors.
- Scope now explicitly includes HGO donor data, projection, ocean adaptation, and cross-domain interaction between water, land, chunks, palette, and zoom rendering.
- Working tree started clean on `main...origin/main [ahead 1]`; latest relevant commit is `39e9a12 亚特兰托帕专项修复`.
- OMX `ultraqa` state could not be re-entered because stale/active `ralph` state blocks runtime overlap in this Codex App surface; this pass uses the UltraQA cycle discipline manually and native read-only subagents for parallel evidence.
- Root causes fixed: ATLSEA projected water now tracks non-empty -> empty political promotion changes; projected ATLSEA feature/signature collection is memoized; detail chunk selection now uses generated per-feature bounds so broad owner chunks with no local features no longer get loaded for Mediterranean viewports; chunk manifest entries now carry per-file `sha256` and strict validation.
- HGO audit: ATLSEA direct donor seas remain local (`directAtlseaIssueCount=0`); ATLISL donor islands remain interactive; the only ATL-owned donor_land is `ATLPRV_18252`, with ATL sand color `#d8c7a6`, so the reported ocean-colored land was addressed through water/chunk routing rather than HGO owner remapping.
- Runtime selection audit after fix: west/central/east Mediterranean required selection dropped to about 30-36MB; full-Med selection dropped from about 55.7MB to about 44.6MB and removed USA/JAP/CAN/ENG/SOV zero-local-feature false positives.
- Verification: Node scenario chunk contracts 25/25 OK; Python chunk assets tests 8/8 OK; strict TNO scenario contract OK; targeted browser e2e `tno atlantropa welded donor islands stay clickable and mediterranean sea uses dedicated fill` passed 1/1 in 3.7m; in-app browser opened the generated validation screenshot.
- Self-check follow-up: fixed the targeted e2e chunk-request helper so repeated owner probes keep the original viewport hook and restore it before the helper-feature probe; strict checker now validates political detail `feature_bounds`; ATLSEA water visual tokens now include projected source and geometry identity to prevent same-id stale water-layer reuse.
- Final verification after review fixes: `node --check tests/e2e/tno_open_ocean_rendering.spec.js` OK; `npm run test:node:scenario-chunk-contracts` 25/25 OK; `python -m unittest tests.test_scenario_chunk_assets -q` 8/8 OK; strict TNO contract OK; browser e2e file ran 2/2 OK after Windows grep quoting widened the selection, including the Atlantropa target in 4.9m.
- Follow-up spot-check: an initial HOI4 strict check showed older non-TNO manifests do not yet carry per-chunk `sha256`/`feature_bounds`; the new mandatory precise-manifest gate is now scoped to `tno_1962`, while other scenarios still validate those fields when present. After the scope fix, strict HOI4 and strict TNO both pass.
- Stop-hook fresh verification: repeated `npm run test:node:scenario-chunk-contracts` 25/25 OK; `python -m unittest tests.test_scenario_chunk_assets -q` 8/8 OK; strict TNO and strict HOI4 OK; `git diff --check` exited 0 with LF/CRLF warnings only. OMX reported no active modes, then stale session-scoped `ultraqa` state files were cleared with `state_clear(all_sessions=true)`.
