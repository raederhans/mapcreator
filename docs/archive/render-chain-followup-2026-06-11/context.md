...
- Post-review hardening now reuses `feature_identity.js` for diagnostic id/country normalization and keeps shell-fallback display-owner detection local to the display-owner diagnostic; the static contract checks shared helper usage, `AL011` geometry/display-owner separation, and the dominant-first, base-map-second, `LAND_FILL_COLOR`-last return chain.
...
- R2 self-review found the test helper duplication risk and addressed it in isolated branch `codex/r2-self-review-fix`; verification is complete and git closeout is tracked by the self-review commit/push.
