...
- `npm run verify:architecture-boundaries` still fails on base file `js/core/map_renderer.js` line budget: 24,154 lines vs 24,100 budget. The same failure reproduces on clean `origin/main@c8f4f24f`.
- `npm run verify:state-write-allowlist` still fails on pre-existing direct state-write allowlist drift outside this diff. The same failure reproduces on clean `origin/main@c8f4f24f`.
...
- Stage5 reached `origin/main@c8f4f24f` first; Layer Observability was then merged in a clean integration worktree from that base.
- Conflict resolution kept the Stage5 registry/archive state, added Layer Observability as an integrated archive row, and regenerated Pages dist from the final source tree.
