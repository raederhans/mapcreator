...
### 29. 顶部 overlay 和 utility 重排后，要立刻做真实点击验证
...
### 30. `$team` / `omx team` 先检查 tmux leader 前置条件
...
### 33. 契约文件名、文档进度和验证脚本必须同名同口径
- 像 `ui_contract.js` / `ui_contracts.js` 这种单复数不一致，会先把验证和留档搞乱，再拖累后续阶段接入。
- 文档里的执行进度、package 脚本名、测试断言要共用同一个 canonical 名称。

### 34. Dev startup must not combine `no-store` with on-the-fly gzip for large JSON/topology assets
...
### 35. JS split 后要警惕同名 helper 的“双声明”，这种错误会在 boot 最早阶段把整页卡死
...
### 36. Readonly startup unlock should only wait for interactions that are truly blocking
...
### 37. Default startup should not silently preload optional scenario layers just because they are visible by default
...
### 38. If startup is bundle-first, shrinking the bundle boundary usually beats local cache tricks
...
### 39. Startup health gate should live inside the scenario apply transaction
...
### 40. Ultra-light runtime shell should keep contract markers and ids, not real mask geometry
...
### 41. UI controller 拆分后，静态合同要跟着切到新的 owner 文件
...
### 42. support surface 这类壳层协调要单独成 owner，别和具体面板实现搅在一起
...
### 43. facade 还在运行链上时，拆 owner 不能顺手带走 facade
...
### 44. 语义地貌层不能按 `atlas_class` 直接 dissolve 成全局 MultiPolygon
...
### 45. 地貌 `blendMode` 不能只在渲染时兜底，必须在 state 层先合法化
...
### 46. Playwright 长超时要输出阶段日志和运行态快照
...
### 46. required check workflow 要 always-run，路径筛选内收成 job 内 classifier
...
### 2. smoke 清单要以实测稳定性为准，不要把深路径硬塞进第一层门禁
...
### 1. 给 Playwright spec 加 tag 前，要先核对 CI 的 grep / grepInvert 规则
...
### 39. workflow `pull_request.paths` 要覆盖 checker 的直接依赖文件
...
### 1. 合成岛屿的 `cntr_code` 和真实 owner 要分层保存
...
### 1. `flushPending` 常用于 idle settle 回路，语义要坚持“只冲刷待处理工作”
...
### 2. synthetic `cntr_code` 和 owner-border 分桶要走两套字段
...
### 1. coastal named water supplement 是否扣 land mask，要允许按 spec 显式声明
...
### 1. `interactive=false` 只能表达不可命中，不能直接表达不可绘制
...
### 1. 线状交通层的运行态诊断要同时接入 draw metric 和 `contextBreakdown`
...
### 1. 场景 strict check 红灯先比对同源 id 集合，再决定重建范围
...
### 1. 水域和 mask 发布前必须走前端同款 D3 / topojson-client 检查
...
### 1. opening-owner border refresh 的最终时点要放在 shell overlay 之后
...
### 2. visual stage 和 deferred infra stage 的 opening-owner refresh 要有单一所有者
...
### 38. post-ready 性能任务要有“交互 quiet window + 单任务诊断归属”
...
### 39. post-ready 调度器要持有 callback 生命周期
