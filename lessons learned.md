...
### 19. runtime merged state 必须区分“没有这个 layer”和“这个 layer 明确为空”
...
### 31. 顶部 overlay 和 utility 重排后，要立刻做真实点击验证
...
### 37. JS split 后要警惕同名 helper 的“双声明”，这种错误会在 boot 最早阶段把整页卡死
...
### 38. Readonly startup unlock should only wait for interactions that are truly blocking
...
### 39. Default startup should not silently preload optional scenario layers just because they are visible by default
...
### 40. If startup is bundle-first, shrinking the bundle boundary usually beats local cache tricks
...
### 46. 语义地貌层不能按 `atlas_class` 直接 dissolve 成全局 MultiPolygon
...
### 47. 地貌 `blendMode` 不能只在渲染时兜底，必须在 state 层先合法化
...
### 48. Playwright 长超时要输出阶段日志和运行态快照
...
### 50. New render passes that depend on scenario masks must be invalidated on runtime mask updates
...
### 51. Startup default scenario apply must participate in the same in-flight gate as manual scenario apply
...
### 52. Chunked coarse preload and noncritical scenario metadata should not block scenario apply completion
...
### 53. 不能把 fill-based physical 机械地下沉到完全不透明的 political 下方
...
### 57. post-ready 渲染链里一个未声明 cache 变量，足以把 exact refresh 和 full hydrate 一起打断
...
### 58. detail promotion 后不能同步全量重建全球内部边界 mesh
...
### 59. chunk promotion 必须把视觉、spatial、hit canvas 当成同一世代提交
...
### 60. 如果默认场景必须首屏正确，就不能先暴露 first-visible-base 预览帧
...
### 61. 海洋 hover 和海洋 click 必须拆成两套策略
...
### 62. 视口相关的内部边界 mesh 不能在 draw pass 里同步补建
...
### 63. chunk refresh 去重逻辑里，千万别把“上一轮选择”先写回再比较
...
### 64. contour 这类大集合 exact pass，先缓存可见集，再谈 draw 性能
...
### 65. exact refresh 不要把“功能开启”误当成“必须强刷”
...
### 66. contextBase 的首个 exact 帧和增强项要分开落地
...
### 67. 懒生成 mesh 的状态不能只靠“数组是不是空”来表达
...
### 68. 任何懒生成后可见的边界，都要立刻同步回 static mesh snapshot
...
### 70. sprite/cache 只要依赖宿主底色，就必须同时接入 key 扩展和 revision 失效
...
### 71. defer / exact-after-settle 只负责冲刷真实 pending，退出 defer 后要补一次受控 flush
...
### 72. contour publish artifacts 不能在生成失败时静默写空文件
...
### 73. viewport 相关缓存 key 不能只看 zoom bucket 和画布尺寸
...
### 74. 查启动性能回归时，必须区分首屏 boot 路径和 post-ready warmup
...
### 75. external context layer 优先走 external pack 和 runtime feature id，不要为了补数据去回写 checked-in topology
...
### 76. `flushPending` 立即执行路径不能先清空 pending 标记再判断要不要启动 refresh
...
### 77. Texture overlay 不能把纸张、线网、标签塞进同一个 render pass
...
### 80. 海域计划不能只验 geometry valid / ID 一致，必须补 probe coverage + seam 检查
...
### 81. 只替换 `scenario_water` 对象而不重整 topology arcs，会把 runtime topo 体积越堆越大
...
### 82. `coverage_invalid_edges` 不能直接拿来判 open ocean coverage 成败
...
### 83. Routine i18n sweeps should audit before full translation sync
- `translate_manager.py` can become disproportionately slow when it rescans every `data/scenarios/**/*.json` file.
- For recurring localization runs, first use `tools/i18n_audit.py` plus targeted geo-locale override checks; only rerun full translation sync when the audit exposes a real gap or `data/locales.json` truly needs regeneration.
### 84. 只刷新 source water 和 runtime topology 不够，startup bundles / manifest 也必须一起重建
...
### 85. 海域第二轮细化优先用 SeaVoX 的 `mrgid_l4` 组团入口，不要一上来把所有 `mrgid_sr` 小湾口全拆开
...
### 86. Inspector 的批量动作必须默认绑定当前可见过滤结果，并且先给影响数量预览
...
### 87. 水域专题接入官方 source 时，要把 base-id 排除、seam 断言和 open-ocean clipping 同波次收口
...
### 90. Marine Regions named-water snapshot 不能混入 `supplement_bboxes`
...
### 91. 海域护栏要单独检查 `marine_macro` 压陆，不能只查 valid / probe / seam
...
### 92. 拆 stage 时，保留的手动 CLI 路径也要一起补齐前置 stage 自恢复
...
### 93. 显式 refresh 不能只进输入 payload，还要直接打断 cache hit
...
### 94. support file 改路径时，要先保证新位置有 checked-in 文件或兼容回填
...
### 95. bootstrap shell 一旦收成空几何，startup bundle 的政治 meta 就必须改从 full runtime topology 生成
...
### 96. coarse chunk 先做 minify + 重复字段收口，往往比先上几何级重构更稳
...
### 97. 对超大 coarse GeoJSON，先比较 raw JSON 和 minified JSON 体积，再决定要不要动字段或几何
...
### 98. checkpoint filename 和 publish scope 如果一起承载目录布局，就必须同步迁移
...
### 99. startup family 瘦身前，要先把消费者矩阵和 section 体积分解做成机器可读报告
...
- `.json.gz` 已经能显著压缩传输体积，删 sidecar 会让启动退化；更稳的最短路径是先固定 consumer matrix、section role、duplication suspects，再决定第一刀切哪一块。

### 100. 对共享 arcs 的 startup topology，先删一个小对象往往只能拿到很小收益
...
### 101. startup topology 里的默认隐藏对象也可能是启动 fallback 契约的一部分
...
### 102. startup bundle 里如果一个 section 已经有稳定外部读取链，就优先把它移出 bundle，而不是继续在 bundle 内做压缩技巧
...
### 103. startup bundle 快路径如果吃 bundle override，缺字段时必须回退外部文件，而不是注入空对象
...
### 104. startup bundle 里凡是按 featureId 重复上万次的 map，优先改成按 runtime feature 顺序编码
...
### 105. `apply_seed` 这类纯派生 startup 便利载荷，先验证运行时 fallback 是否已足够，再决定要不要序列化
...
### 106. startup support 的 key 裁剪不能只靠 topology object id + patch key 推导
...
### 107. startup support 的真实白名单最好从 `i18n` 集中 lookup 入口采，不要继续只靠静态 topology 推导
...
### 108. 只采到 1 次 startup support runtime 样本时，generator 只能产候选白名单，不能直接反推正式裁剪规则
...
### 109. generator 和 candidate materialization 不能并行读写同一个 whitelist 文件
...
### 110. 正式 support slimming 第一版要把 whitelist 当成“带护栏的正式输入”，但 post-slim 样本仍然必须回采
...
### 111. scenario-scoped support slimming 输入不能偷用某个 scenario 的默认白名单
...
### 112. 只把新输入接进 stage signature 还不够；默认 rebuild domain 也要走到对应 stage
...
### 113. post-slim 第二版 whitelist 校正默认应“baseline whitelist ∪ 新样本回补”，不能直接用新样本覆盖
...
### 114. startup support 剩余 miss 要先判“是不是这个系统该管的”，再决定要不要继续加白名单
...
### 115. 主运行时已经切到 scenario-scoped 后，要尽快删 root legacy 文件并同步修测试夹具
...
### 116. canonical whitelist 这种参与签名的正式输入，不能混入机器本地 provenance
...
### 117. 采样 harness 不能把启动阶段已采到的 audit state 清掉，报告文件名也要带上足够维度避免互相覆盖
...
### 118. 场景 relief overlay 的首帧异常，先查“默认视觉层本来就在画什么”，不要先怀疑 startup/detail 数据链
...
### 119. 首屏间歇性视觉脏块，如果只在 startup/coarse 阶段出现，除了改颜色，还要考虑推迟那类 overlay 到 detail 稳定后再显示
...
### 120. Pages/部署构建如果会在 CI 里重建 `dist`，就不要把 `dist/app/data/**` 这类生成产物塞进 Git 历史
...
### 121. startup support slimming之后，新增顶层功能入口先默认只进 deferred runtime，不要顺手塞回 startup 主链
...
### 122. 总显隐开关不能顺手覆写子图层可见性，尤其当保存链会把子图层状态当成真值写盘时
- 这次 Transport Overview 暴露出一个典型坑：如果总开关关闭时直接把 showAirports/showPorts 改成 false，项目保存后就会把“临时总隐藏”误写成“family 真的关闭了”。
- 更稳的做法是：总开关只负责渲染门控；family 自己的可见性继续单独保存，必要时再单独持久化“上次展开状态”，不要混用同一组字段。
### 123. 未来 family 的占位状态如果还没接通 runtime，就不要先写进项目保存链
...
### 124. 场景 overlay cache signature 不能只看数据数量，还要带上 detail/composite 阶段 token
...
### 125. startup bundle 先进入 ready 不代表场景视觉壳已经完整，缺 mask 的场景要提前 full hydration
...
### 126. 全球静态交通 builder 不能先把全量世界主干网物化到 Python list，再做过滤和简化
...
### 127. 多产物地理 builder 不能把 preview / full / labels 绑成同一波总装
...
### 128. 串行 staged output 只能先解决波次耦合，不能自动解决最终 materialize + TopoJSON 汇总压力
...
### 129. 给超大全球 builder 补阶段日志，先分清是卡在扫描、归一化，还是卡在最终总装
...
### 130. 全球 Overture builder 先要判断“卡在扫描归一化”还是“卡在最终总装”，不要把两类瓶颈混在一起
...
### 131. 大文件远端扫描别轻易加大 Arrow read-ahead，先验证内存曲线
...
### 132. 把产品口径继续收窄到 motorway+trunk 也不一定够，必须看几何处理链本身是不是主瓶颈
...
### 133. 全球分片 builder 要按密度分片，不要只按等宽经度切
...
### 134. 高密区分片最好支持“固定密度 shard + 自定义经度窗口”双轨
...
### 135. 对分片构建失败的半成品目录要及时清掉，避免把“只有 recipe”的失败尝试当成 ready shard
...
### 136. 线几何简化后必须立刻重算长度，并重新应用基于长度的阈值
- reveal_rank、preview/full 过滤、导出字段只要依赖 length_m，就不能继续使用简化前长度；否则产物会和最终几何不一致。
- 更稳的最短路径是：simplify -> measure_lengths -> 再做长度阈值过滤和分级。

### 137. 城市点位 e2e 一旦依赖 labelEntries，就必须先等 exact render 稳定并在测试里显式打开 showLabels / labelMinZoom
- 这次 city reveal 回归在单独跑能过、整组跑会偶发归零，根因不是 reveal 算法本身，而是 `buildCityRevealPlan()` 的 label 产出受 state.deferExactAfterSettle 和 styleConfig 开关控制。
- 更稳的做法是：凡是断言 labelEntries 或 label density 差异的 Playwright 测试，都先等待 renderPhase=idle && !deferExactAfterSettle，并在测试配置里显式给出 showLabels: true、合适的 labelMinZoom。

### 138. 延迟加载的数据一旦会反推已挂载 UI 摘要，就要补显式 UI refresh 钩子
...
### 139. 编辑器里的精确 overlay 点击，必须先按“相关面板是否真的在用”做门控
...
### 140. 一旦给已有视觉家族补独立主色，原来的“强度滑块顺手改色”就必须立即收口
...
### 143. 信息卡 polish 的第一刀优先做减法：去冗余、分主次、降重量
...
### 144. 渲染 helper 里新增样式字段时，必须显式走参数链，不要偷读调用侧局部变量
...
### 145. 命中缓存一旦在每次重绘都会重建，选中态就必须同步重绑到新 entry
...
### 146. 浮动详情卡的宿主 surface 一旦被关闭，卡片和高亮必须同步撤场
...
### 147. 新增 UI 文案时，凡是 `t()` 动态拼出来的 key 都要和静态 DOM 文案一起补齐
...
### 148. 早期 capital label phase 不能继续复用城市点位的 minZoom 门槛
...
### 149. 连续叠加同一 feature 的 PR 时，先收口唯一 state schema，再继续加 UI 和导出分支
...
### 150. checked-in shard 目录不能靠“看起来差不多”存活，必须和 builder 真相逐项对齐
...
### 151. 未来全局 transport 家族在正式产物没落地前，不要先挂进默认 eager loader
...
### 152. 长扫描 builder 只在 flush 时打日志还不够，scan checkpoint 必须早于首次产物写出
...
### 153. 支持区工具一旦升级成一级功能区块，必须同波次迁移 URL restore、旧壳清理和契约测试
...
### 156. manifest 输出层级一旦改成嵌套目录，共享 contract discovery 必须同波次递归化
...
### 157. preview shard 总量足够小时，runtime 优先一次性 lazy load 全部 preview，而不是提前做 viewport shard 调度
...
### 158. 分阶段开放 runtime 时，要用测试把“主地图已开 / save-load 未开”这个边界钉死
...
### 159. 共享阈值 helper 不能直接套到 reveal_rank 语义不同的交通家族上
...
### 163. 继承的 deny_tags 不能压过子剧本里显式确认的 verified 映射
...
### 164. reviewed 映射层和 runtime 默认色桥拆开后，生成器与消费端都要同时认同一份白名单语义
...
### 165. 最后专题项如果依赖非 runtime 锚点，优先保留专题状态，不要为了清零 unmapped 强行并到大国锚点
...
### 166. runtime default bridge 一旦引入 `expose_as_runtime_default`，scenario 主地图颜色桥也要同波次切到同一份 canonical 语义
...
### 167. active scenario 的 tag 颜色不能复用 ISO2 runtime default bridge
...
### 168. TNO 静态颜色修正要放一个最终 audit 对齐口
...
### 172. benchmark 串味收口后，如果 HOI4 首帧仍然接近 12s，就该直接盯 `drawPoliticalPass`
...
### 173. 只要 scenario 已有 chunked political runtime，apply 前就别再强等 detail topology
...
### 174. benchmark 汇总层必须把“same-scenario fresh metric”当成硬门槛
...
### 175. full-pass Canvas 背景合批要缓存“可重放结果”，只缓存 entry 列表收益不够
...
### 176. benchmark 主动触发型指标必须满足运行时前置条件，探针本身也要和产品逻辑同构
...
### 177. 视觉阶段指标和 runtime 收尾指标要分开看，用户体感优先用 visual stage
...
### 178. 首帧 coarse gate 和 post-frame detail prewarm 必须拆成两条显式合同
...
### 179. 内部 loader 模块首轮下沉时，先抽纯加载合同和 metadata helper，facade 继续持有 active state / UI side effect
...
### 180. 边界测试如果把 helper 所有权钉死在 donor 文件上，模块下沉后会先炸测试，再误导重构判断
...
### 181. 对超大 donor 文件做批量文本替换时，先锁定更窄上下文，再动同名局部变量
...
### 182. 当 runtime controller 和 loader helper 共享同一份单例状态时，用 late-bound 回调接线最稳
...
### 183. 当 hydrate controller 需要回调 facade 的 load 函数时，也用晚绑定保住出口稳定
...
### 184. apply pipeline 下沉后，旧边界测试要改成“事务 owner”和“状态 owner”两层断言
...
### 185. 模块下沉后，facade 的本地 wrapper 和新 import 必须先做符号去重
...
### 186. 提取 controller 时，旧 donor 文件里继续会用到的 helper 依赖要显式注入到新模块
...
### 187. UI 大文件首刀先拆“闭环面板”和“纯错误处理”，toolbar 继续保留 state callback 注册
...
### 188. overlay 类 UI 第二刀先拆“面板内部闭环”，把跨 surface 仲裁和 URL restore 留在 toolbar facade
...
### 189. special zone 这类编辑面板要把“面板闭环”和“popover 外壳”分两层拆
...
### 190. export workbench 这类大面板要先抽 controller，再保留 toolbar 的 overlay facade
...
### 191. 抽离闭环 UI controller 时，owner 的 schema/default/state helper 要整块同迁
...
### 192. UI 面板拆分时，texture 和 dayNight 这种相邻功能也要按真实事务语义分组
...
### 193. 当一个独立 editor 仍然嵌在大面板里时，toolbar 继续保留 host wrapper 最稳
...
### 194. UI owner 下沉时，微型 history helper 要和事件绑定一起迁走
...
### 195. 大型 inspector 拆分时，先让 donor 保留 runtime 容器，再把 owner 逻辑下沉
...
### 196. strategic overlay 的 history snapshot 要把 `operationalLines` 和 dirty flag 一起纳入
...
### 197. project import owner 可以下沉，但 overlay invalidation hook 必须继续显式注入
...
### 198. 大块搬走 UI owner 时，要先点名 donor 里仍然复用的 shared helper
...
### 199. controller 下沉后，selection 和快捷条上的 donor 绑定要单独复核
...
### 200. 宿主 panel 的 DOM 引用和 category 显隐合同要成对保留
...
### 201. 带有 draft model 和模板保存链的 editor，拆分时要把整条事务链一起迁走
...
### 202. 大型宿主壳拆分时，DOM builder 和 dock chrome 要同波次迁走
...
### 203. 纯配置模块拆分时，先做 compat re-export，再补跨文件合同测试
...
### 204. donor 自己在初始化期要用到的 defaults，必须继续显式 import
...
### 205. 启动壳拆分时，先搬纯 startup helper，再让 main.js 保留状态推进
...
### 206. 启动 overlay 拆分时，用 controller 收住内部句柄最稳
...
### 207. donor 读取 controller 内部常量时，要改成显式 API
...
### 211. 大文件拆分后，导出面和 wiring 缺口会沿启动链串联放大
...
### 212. boot overlay 的测试等待条件要以真实 boot state 为准，不能只盯 overlay hidden/aria-busy
...
### 213. render helper 拆分时，dirty/signature/history owner 继续留在 donor 最稳
...
### 214. 启动链 data pipeline 可以下沉，但 boot 编排必须继续留在 main
...
### 215. startup scenario boot 拆分时，bundle apply recovery 可以下沉，ready-state 编排继续留在 main
...
### 216. detail promotion 拆分时，transaction 可以下沉，ready-state 分支判断继续留在 main
...
### 217. state singleton 拆分时，优先下沉默认 state slice factory，再让 donor 保留单例壳
...
### 218. runtime hook 拆分时，要把“已声明的 hook”和“运行时动态挂上的 hook”一起显式化
...
### 219. URL 状态拆分时，query helper 下沉，restore 编排继续留在 support-surface controller
...
### 220. donor import 面收窄后，旧边界测试要同步改成 owner 接线合同
...
### 221. Playwright 里的 source-level 合同检查要优先读 repo 文件，不要依赖页面 fetch 当前源码
...
### 222. clear 场景时，detail ready 状态要看 `topologyDetail`，不要拿 runtime topology 代替
...
### 223. 拆 bundle/cache owner 时，让 facade 继续持有对外 export 和 getter 桥接最稳
...
### 224. strategic overlay 默认形状要有单一真源，尤其是 `unitCounterEditor`
...
### 225. scenario runtime 默认 shape 也要让 reset / rollback / health 共用同一组 factory
...
### 226. map_renderer 后续拆分先抽 mesh builder，再动 draw 和 render pass
...
### 227. border draw 拆分先搬纯 helper，draw 主体后移一刀更稳
...
### 228. drawHierarchicalBorders 主体下沉时，要把共用 coastline simplify 链继续收口在 donor facade 上
...
### 229. interaction border snapshot 这类“离屏缓存小事务”适合独立 owner，drawBordersPass 继续留 donor
...
### 230. spatial/index runtime 拆分时，索引构建可以下沉，runtime refresh 交易继续留 donor
...
### 231. 抽 shared presentation owner 时，要把 stateless parser 和 stateful runtime 一起分层
...
### 232. startup runtime topology baseline 不能在 scenario apply 里二次播种
...
### 233. 做 state owner 拆分时，合同测试要直接封住 consumer 里的第二份默认 shape
...
### 234. renderer runtime / border cache / spatial index 要按“默认 shape + 复位点”一起收口
...
### 235. opening-owner mesh 的 mesh-pack 直用路径要和 runtime fallback 路径分开判断
...
### 236. startup 场景 ready 判定要包含 opening-owner cache，就绪条件只看 mesh pack 会放过半完成状态
...
### 237. Playwright 的 `waitForFunction(async ...)` 会把 Promise 本身当成 truthy，ready gate 必须改成同步轮询
...
### 238. 收紧 namespace import 时，要把仍然向 controller 传递的 helper bag 一起显式化
...
### 239. 把 read helper 下沉到 owner 时，要把原本顺手做的 state 初始化副作用一起带走
- 这次 getUnitCounterPreviewData() 迁进 strategic overlay runtime owner 后，最先丢的不是返回值计算，而是 ensureUnitCounterEditorState() 这类播种和归一化副作用。
- 更稳的最短路径是：只要公开 facade 过去会顺手初始化 editor state，owner 版实现也要先保住这层副作用，再谈纯读 helper。

### 240. strategic overlay 的浏览器回归要对齐真实 DOM 壳层，不要继续假设旧的 Frontline tab
...
### 241. project roundtrip full regression 里，schema 和 annotationView 断言要跟着真实导出契约走
...
### 242. strategic-only roundtrip regression 适合直连 import transaction，不必绑死 file input 壳层
...
### 243. Playwright 遇到 hidden overlay 拦截点击时，优先走 DOM 侧 value+event 驱动
...
### 246. Playwright 的 project import watch 更稳的做法是页面轮询 debug snapshot，不要把整段完成判定塞进 `page.waitForFunction`
...
### 247. state write allowlist 必须和检查器共用同一套扫描逻辑，不能一边用 rg，一边用另一套正则
...
### 248. startup scenario apply 不要串行等待 deferred UI bootstrap
...
### 249. Playwright 的 `boundingBox()` 不适合直接驱动深度 transform 的 SVG 交互
...
### 250. startup 里的并行 promise 不能只挂全局引用，必须保留本次启动的局部 promise
...
### 251. startup cache key 的输入要走显式参数，不要让 data loader 偷读全局 state
...
### 252. scenario Playwright 长测里，资源加载超时和 UI 操作路径要分开定性
...
### 253. scenario rollback snapshot 要覆盖资源 owner 和 capability，不能只回滚 id 与 assignment map
...
### 254. same-scenario early return 要看 cached bundle 的完整就绪条件，不要只看 active id 和几张 map
...
### 255. scenario single-flight 的 promise 要先建立，再同步 UI
...
### 256. bundle load 也需要按 scenarioId + bundleLevel 做 in-flight 复用
...
### 257. 延迟到下一帧的 reset 后处理要自己补 render
...
### 258. renderer 大门面继续拆时，先收 state 写口，再动 facade 和调用链
...
### 259. hydration health gate 的主合同要以当前运行时语义为真源，恢复链测试要跟随统一
...
### 260. perf gate 的 render median 对偶数样本必须取两中位数均值
...
### 261. 批量把 `state` 改名时，导入路径和标识符要分开替换
...
### 262. 把函数指针改成 helper 调用时，要保留“无 handler 时的本地回退”
...
### 263. state write guardrail 既要识别 computed write，也要排掉本地测试/缓存对象的 `state` 噪音
...
### 264. owner factory 的参数名从 `state` 改成 `runtimeState` 后，要把所有调用点一次性跟上
...
### 265. contract test 要跟随真正 owner，别靠注释锚点维持假边界
- helper 下沉后，旧测试如果继续只扫主文件，就会逼出注释锚点这类临时补丁。
- 更稳的做法是让 contract test 同时检查 facade 文件和新 owner/helper 文件，直接表达真实边界。

### 266. 源码正则 gate 应优先约束语义结构，少绑定局部变量名
- `state` 改成 `runtimeState` 这类重命名不该触发结构性回归误报。
- 对源码 contract，优先匹配调用顺序、关键 API、关键条件组合，少把变量名写死。

### 267. owner 只做 composition 时，合同要把 helper -> domain 注入关系直接钉住
- 这次 unit counter 下沉后，真正脆弱的边界不在 facade 名字，而在 `resolveUnitCounterNationForPlacement: unitCounterHelpers.resolveUnitCounterNationForPlacement` 这类显式 wiring。
- 更稳的最短路径是：boundary contract 同时检查 owner 的 spread surface 和关键 helper 注入行，避免 domain 因漏接线静默退回默认实现。

### 268. required check workflow 要 always-run，路径筛选内收成 job 内 classifier
- 这次 `perf-pr-gate` 用 `pull_request.paths` 触发，性能无关 PR 上 required check 会直接缺席，分支保护无法稳定收口。
- 更稳的最短路径是：workflow 本身始终触发，job 内先做 changed-files classifier，再决定走真实 gate 还是快速成功。
- 这样 branch protection 看见的 check 名始终稳定，审计信息也能跟着同一条 required job 留下。

## 2026-04-23 - smoke happy-path 收口

### 269. smoke i18n 优先锁语言状态和最小可见文案
- 主壳 i18n smoke 直接盯 `currentLanguage`、左右面板按钮文案、语言按钮文案，稳定性比长串 `toHaveText` / `not.toHaveText` 更高。
- 这样能把“文案切换活着”与“整页所有静态文案都对”分开，红灯信号也更聚焦。

### 270. smoke 清单要以实测稳定性为准，不要把深路径硬塞进第一层门禁
- 这次 strategic overlay 相关 smoke 在当前仓库里持续超时，直接塞进第一层 smoke 只会把门禁变成噪音。
- 更稳的最短路径是：保留同主题覆盖目标，但把第一层 smoke 收到能稳定通过的主路径 spec；更深的 strategic 路径继续留在 feature / regression 面。

## 2026-04-23 - 静态 review 留档

### 271. 给 Playwright spec 加 tag 前，要先核对 CI 的 grep / grepInvert 规则
- 这次 `@dev` 一加上去，`playwright.config.cjs` 里的 `grepInvert: process.env.CI ? /@dev/ : undefined` 会让整条 spec 在 CI 中直接消失。
- 更稳的做法是先确认这条测试是否真的应该退出 CI，再决定用 tag、独立项目，还是独立命令。

### 272. 把静态合同从 E2E 拆到 node:test 时，要同步接到现有具名入口
- 新合同文件如果只落在仓库里，`package.json` 和 workflow 还指向旧文件，路径稳定性和 regex 降耦改进就不会生效。
- 更稳的最短路径是直接替换原有具名入口，或者把新文件并入原命令。

### 273. Playwright `testIgnore` 的路径要锚定到真实测试目录
- 这个仓库绝对路径里带有 `Desktop\\dev\\...`，如果忽略规则只写宽泛的 `dev/**` 或 ` /dev/ ` 片段，`CI=1 playwright --list` 很容易把整棵测试树一起误伤。
- 更稳的最短路径是直接锚定到 `tests/e2e/dev/` 这一层，比如 `testIgnore: /[\\/]tests[\\/]e2e[\\/]dev[\\/]/`，这样 root 下普通 `dev_*` 文件还能保留。

### 274. workflow `pull_request.paths` 要覆盖 checker 的直接依赖文件
- 只监听入口脚本和数据目录，会让共享契约或配置改动绕过验证；像 `tools/check_scenario_contracts.py` 这种 checker，直接 import 的 `map_builder/config.py`、`map_builder/contracts.py` 也要进触发范围。
- 最稳的做法是每次给仓库加新的校验 workflow，都顺手核一遍脚本的直接 import 依赖，再决定 `paths` 白名单。

## 2026-04-23 - ATLISL 静态链路梳理

### 275. 合成岛屿的 `cntr_code` 和真实 owner 要分层保存
- `ATLISL_*` 的 runtime 分桶看 `cntr_code`，真实归属看 `owners.by_feature.json`；把 owner 覆盖顺手写回 `cntr_code`，会直接把同一批岛屿拆进 `ita/ibr/tur` 等 detail chunk。
- 更稳的做法是：岛屿生成端固定 `cntr_code="ATL"` 作为 synthetic group，owner/controller/core 继续走 assignment map。

## 2026-04-23 - TNO startup / runtime topology 收口

### 276. runtime topology stage 返回 full state 时，要把 water stage artifact 一起带回
...
## 2026-04-23 - chunk flush / border bucket review 修复

### 277. `flushPending` 常用于 idle settle 回路，语义要坚持“只冲刷待处理工作”
- 这次 `render-phase-idle` 每次都会带 `flushPending:true` 进入 chunk runtime；如果 `flushPending` 被放宽成“无条件启动 refresh”，所有 chunked scenario 都会在每次 settle 多跑一轮 selection/registry 检查。
- 更稳的最短路径是：ready 首次提交单独 seed 一次 pending work，`flushPending` 本身继续只负责冲刷已经挂起的 refresh/promotion。

### 278. synthetic `cntr_code` 和 owner-border 分桶要走两套字段
- `ATLISL_*` 这次需要保留 `cntr_code="ATL"` 给 runtime chunk/topology 分桶，同时 border mesh、visible border country、detail/internal border grouping 需要跟随显示 owner。
- 更稳的最短路径是：空间索引额外带一个 `borderMeshCountryCode`，border mesh 相关 helper 全部读 owner-aware bucket，交互和 canonical country 逻辑继续读原始 `countryCode/cntr_code`。

## 2026-04-23 - TNO water validator 定点修复

### 279. coastal named water supplement 是否扣 land mask，要允许按 spec 显式声明
- 这次 Cardigan Bay 的 probe miss 根因是 coarse land mask 先把补片框吃掉了，named water source 和 supplement 都是对的，真正错的是把所有补片一律当成‘必须服从 land mask’。
- 更稳的最短路径是：supplement 默认继续扣 land mask，但对少数经过 validator 固定点验证的海湾/海峡，允许 spec 显式关闭这一步，让 named water 几何成为该点位的权威来源。
## 2026-04-23 - TNO 海洋统一拓扑收口

### 280. 长构筑链先用轻量几何脚本验证规则层，再决定是否整链重跑
- 这次 probe / seam / land-overlap 的最后收口，先用 named-water 轻量脚本直接验证规则层，比反复整链跑 `runtime_topology` 快得多。
- 更稳的最短路径是：先确认局部几何规则已经全绿，再只跑最小必要的 chunk / final validator。

## 2026-04-23 - Atlantropa 可见性与交互拆分

### 281. `interactive=false` 只能表达不可命中，不能直接表达不可绘制
- 这次 `ATLISL_* boolean_weld` 是可见的重建岛屿，但需要退出 hit / country fill / parent target；把它们统一塞进 interaction exclude 会让绘制链也丢岛。
- 更稳的最短路径是：建立 visual exclude 和 interaction exclude 两条链，绘制/背景/边界走 visual，hit/批量操作/选择目标走 interaction。

## 2026-04-23 - Transport deferred context visibility

### 282. 线状交通层的运行态诊断要同时接入 draw metric 和 `contextBreakdown`
- 这次 Road/Rail 已有绘制函数，但 staged skip metric 与 `CONTEXT_BREAKDOWN_METRIC_NAMES` 都缺少线状交通层，运行态探针会少一半关键信号。
- 更稳的最短路径是：新增 context draw 层时同步检查 staged skip metric、`contextBreakdown` allowlist、用户显式释放 deferred pass 的路径。

## 2026-04-23 - tno_1962 strict contract data sync

### 283. 场景 strict check 红灯先比对同源 id 集合，再决定重建范围
- 这次 runtime topology 和 chunks 已经一致，manifest 计数也等于 runtime 去掉允许 fallback 后的数量，真正过期的是 feature maps 与 capital hints manifest 字段。
- 更稳的最短路径是：先找同源 checkpoint 验证 owners/controllers/cores 对当前 runtime 的差集为零，再只同步 feature maps 和相关 manifest 字段。

## 2026-04-23 - TNO D3 spherical geometry

### 284. 水域和 mask 发布前必须走前端同款 D3 / topojson-client 检查
- Shapely valid 的 polygon 仍可能被 D3 解释成 full sphere，Python topojson 还会重写 winding，不能代表前端真实行为。
- 更稳的最短路径是：source GeoJSON 用 D3 检查，runtime TopoJSON 先用 `vendor/topojson-client.min.js` 转 FeatureCollection，再用 D3 检查 world-bounds、sphere area 和无关 probe。

## 2026-04-24 - Phase 1 refresh contract static review

### 285. opening-owner border refresh 的最终时点要放在 shell overlay 之后
- `refreshScenarioOpeningOwnerBorders()` 的缓存键包含 `scenarioShellOverlayRevision`，fallback 路径如果在 `setMapData()` 后就把它当成“已经刷新过”，shell overlay 改动一落地就会留下旧边界。
- 更稳的做法是：把最终一次 opening-owner refresh 固定在 `refreshScenarioShellOverlays()` 之后；恢复路径允许多做一次，也要守住最终状态正确。

### 286. visual stage 和 deferred infra stage 的 opening-owner refresh 要有单一所有者
...
- landing 这类展示页加 reveal 时，CSS 默认应保持内容可见，再由 JS 显式开启轻量动效；否则 full-page 截图和低速设备会看到大块空白。
- 新增 i18n 可见文案和 aria label 时，要同步补 source-level contract，避免只在 dist 构建后才发现漏翻译。

### 2026-04-24 - App/web i18n coverage
- i18n audit 要把 `INLINE_UI_TRANSLATIONS` 当成运行时可用翻译源；只看 `data/locales.json` 会把已由 inline 覆盖的主壳文案误报成缺失。
- 页面级 i18n 新增 `alt` 或 aria 文案时，要同步更新 source contract，避免图片替代文本和导航标签在中文模式下继续停留英文。

### 2026-04-24 - i18n catalog / audit modularization
- i18n audit 和 translate manager 的 key 发现规则必须共享同一套 extractor；抽出 descriptor 或 catalog 后，如果只改 audit，翻译生成链会漏收新增文案。

### 288. post-ready 性能任务要有“交互 quiet window + 单任务诊断归属”
- ready 后的 deferred/context/infra 工作很容易和用户第一轮缩放撞车；调度层要用 renderPhase、isInteracting、zoom quiet window 一起判定。
- 如果诊断只有一个 active task 槽，post-ready 任务执行也要串行化，否则 long-animation-frame 会把责任归到错误任务。
- transformed fast frame 先检查缓存完整性再清 canvas，fallback 帧不能再次写回 last-good，避免把旧画面越缓存越脏。

### 289. post-ready 调度器要持有 callback 生命周期
- callback 启动异步任务时必须返回 promise，让 active task 诊断槽覆盖真实执行期。
- 调度器统一吸收同步抛错和 async rejection，避免 active task 永久卡住或产生 unhandled rejection。

### 290. chunk runtime E2E 的“曾经加载”和“最终仍加载”要分开验
- zoom-end 测试里 detail chunk 可能先满足 waitForFunction，随后被 idle/refresh 选区收回；断言应分别记录曾经命中的 detail chunk 与最终可见状态，避免把调度竞争误判成单点功能失败。


### 291. async promotion 切片要把事务边界放在 mutation 前
- wait 放在 visual mutation 之后会让 rollback 或新 selection 看到半提交状态；切片点应落在 mutation 前，或者配完整回滚快照。
- single-flight 期间的新 refresh 要记录为 post-commit replay，直接清 pending 会丢用户刚触发的 zoom-end 选区。
...
### 2026-04-27 - 切片调度和 worker raster 身份必须有行为契约
- exact-after-settle 这类多段切片调度要让单个 task 异常可见且继续排空后续队列，否则 controller 会卡在 applying/scheduled。
- worker raster 的 currentness key 要覆盖 viewport；scenario/version/color/transform 相同但 viewport 改变时，旧 ImageBitmap 结果会画到错误区域。

### 2026-04-26 - 性能静态 contract 也要有命名入口
- benchmark 相关 contract 如果只靠临时 `python -m unittest ...` 命令触达，入口会慢慢漂移；最稳的最短路径是给现有 Python perf contract 补一个 package.json 命名脚本，并让 editor benchmark 复用固定 shell 入口。
...
- 更稳的做法是同一轮更新脚本合同、Python contract、checked-in baseline，并用 `perf:baseline` 后接 `perf:gate` 验证。

### 2026-04-29 - chunk manifest 成本字段要对真实文件设合同
- `byte_size` 这类 runtime selection 成本字段只改 manifest 会让调度成本和真实 payload 脱节。
- 更稳的做法是让 checked-in manifest 对真实 chunk 文件 `stat().st_size` 有合同测试；生成器继续以真实文件大小为唯一来源。

### 2026-04-29 - transport appearance 可见性要同时看 loader 白名单和 renderer rank 门
- catalog-backed context layer 如果不在普通 pack 表里，显式请求也要有自己的 allowlist；否则 toggle 会显示开启，但 loader 分支永远进不去。
- 面板 summary 的过滤 rank 和 renderer 的 zoom reveal floor 要按同一批真实数据校准；当前数据最高 rank 只有 2 时，默认显示门设为 3 会形成“已加载但 0 可见”的假完成。

### 2026-04-29 - startup bundle 可复现性要用同源发布数据验证
- runtime topology、ownership maps、bootstrap topology 必须来自同一批发布数据；混用旧 checkpoint 会触发缺失 feature id，生成器的严格报错应保留。
- full runtime rebuild 成本很高时，先用已发布同源数据直接跑 startup bundle 生成器验证可复现性，再把重构建内存问题作为独立构建链债务处理。
...
### 293. Interaction benchmark idle must include runtime chunk work
...
### 294. 测试路由 selector 要把“列表文件”和“变更文件”分成两个入口
...
### 2026-05-01 - city-runtime shared boot gray rollout
- worker 级 shared boot 要把 profile key 和 boot path 分开保存；fixture 里只能传相对路径给 gotoApp()，绝对 URL 再次包一层会把 /app/ 前缀拼成错误的双重地址。
- TNO 相关 shared prep 不能复用最严格的 shell-owner gate；city-runtime 灰度更稳的等待边界是 waitForAppInteractive + waitForShellReady + waitForScenarioApplyIdle + exact render idle。
- Windows 下 adaptive runner 执行 npm script 时，要显式走 cmd.exe /c npm run ...；直接 spawnSync('npm.cmd') 会得到 EINVAL，本地 dry-run/execute 会失真。

### 2026-05-01 - shared boot reset must restore mutable city datasets
