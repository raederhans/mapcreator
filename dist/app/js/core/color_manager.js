...
class ColorManager {
  ...
  static regionColorCacheLimit = 512;
  static labCacheLimit = 2048;
  static ownerColorCache = {
    signature: "",
    result: null,
  };

  static trimCacheMap(cacheMap, limit) {
    if (!(cacheMap instanceof Map)) return 0;
    const normalizedLimit = Math.max(0, Number(limit || 0));
    if (normalizedLimit === 0) {
      const cleared = cacheMap.size;
      cacheMap.clear();
      return cleared;
    }
    let removed = 0;
    while (cacheMap.size > normalizedLimit) {
  ...
  static clearRuntimeCaches() {
    ColorManager.regionColorMap.clear();
    ColorManager.labCache.clear();
  ...
  static getRuntimeCacheSnapshot() {
    return {
      regionColorEntries: ColorManager.regionColorMap.size,
      regionColorCacheLimit: Math.max(0, Number(ColorManager.regionColorCacheLimit || 0)),
      labCacheEntries: ColorManager.labCache.size,
      labCacheLimit: Math.max(0, Number(ColorManager.labCacheLimit || 0)),
      ownerColorCacheReady: !!ColorManager.ownerColorCache.result,
      ownerColorCacheSignature: String(ColorManager.ownerColorCache.signature || ""),
    };
  }
  ...
  static getRegionColor(tag) {
    ...
    ColorManager.trimCacheMap(ColorManager.regionColorMap, ColorManager.regionColorCacheLimit);
    return color;
  }
  ...
  static colorToLab(hex) {
    ...
    ColorManager.trimCacheMap(ColorManager.labCache, ColorManager.labCacheLimit);
    return lab;
  }
