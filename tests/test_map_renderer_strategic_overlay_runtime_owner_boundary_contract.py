...
class MapRendererStrategicOverlayRuntimeOwnerBoundaryContractTest(unittest.TestCase):
    def test_map_renderer_keeps_overlay_render_orchestration_while_facade_hosts_pass_through_wrappers(self):
        ...
        self.assertIn("function insertOperationGraphicVertex(insertIndex = -1, coord = null) {", operation_graphics_content)
        self.assertIn("function beginOperationGraphicVertexDrag(vertexIndex = -1) {", operation_graphics_content)
        self.assertIn("function moveOperationGraphicVertexDrag(vertexIndex = -1, coord = null) {", operation_graphics_content)
        self.assertIn("function finishOperationGraphicVertexDrag(vertexIndex = -1) {", operation_graphics_content)
        ...
        self.assertNotIn('markDirty("insert-operation-graphic-vertex")', renderer_content)
        self.assertNotIn('kind: "move-operation-graphic-vertex"', renderer_content)
        self.assertNotIn('markDirty("move-operation-graphic-vertex")', renderer_content)
        self.assertIn("function commitSpecialZoneMembershipClick({", owner_content)
        self.assertIn("function beginSpecialZoneMembershipDrag({", owner_content)
        self.assertIn("function applySpecialZoneMembershipDragFeature(featureId = \"\") {", owner_content)
        self.assertIn("function finishSpecialZoneMembershipDrag() {", owner_content)
        self.assertNotIn("function applySpecialZoneMembershipFeature(featureId, mode, layerId = \"\") {", renderer_content)
        ...
        self.assertNotIn("markDirty(`special-zone-membership-${mode}`)", renderer_content)
