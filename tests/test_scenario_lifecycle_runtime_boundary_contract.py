...
class ScenarioLifecycleRuntimeBoundaryContractTest(unittest.TestCase):
    def test_lifecycle_runtime_owns_reset_clear_transaction(self):
        ...
        self.assertIn('runtimeState.topologyBundleMode = hasBaselineDetailTopology ? "composite" : "single";', content)
        self.assertIn("runtimeState.detailDeferred = hasBaselineRuntimeTopology && !hasBaselineDetailTopology;", content)
        self.assertIn("runtimeState.detailPromotionCompleted = hasBaselineDetailTopology;", content)
        self.assertIn("restoreScenarioOceanFillAfterExit();", content)
        self.assertIn("restoreScenarioDisplaySettingsAfterExit();", content)
        self.assertIn("runtimeState.scenarioReleasableIndex = createDefaultScenarioReleasableIndex();", content)
        self.assertIn("restoreScenarioHydrationHealthGateState(", content)
        self.assertIn("restoreScenarioDataHealthState(", content)
        self.assertNotIn("runtimeState.scenarioHydrationHealthGate =", content)
        self.assertNotIn("runtimeState.scenarioDataHealth =", content)
