...
test("exact-after-settle keeps scenario overlays on the contextScenario reuse path", () => {
  ...
  const contract = {
    ...
    exactAfterSettleSuccessInvalidatesPoliticalPass:
      /function applyExactAfterSettleRefreshPlan\(plan\) \{[\s\S]*?runtimeState\.deferExactAfterSettle = false;[\s\S]*?invalidateRenderPasses\("political", "exact-after-settle-political"\);[\s\S]*?plan\.politicalInvalidationReason = "exact-after-settle-political";[\s\S]*?plan\.politicalInvalidatedAt = politicalInvalidatedAt;[\s\S]*?const targetPassNames = new Set\(\["political", "borders", "labels", "textureLabels"\]\);/.test(rendererSource)
...
test("Atlantropa field-driven interaction contracts preserve explicit render and hit layers", () => {
  ...
  const checks = {
    ...
    politicalPromotionTreatsAtlantropaLayerAsWaterChange:
      ...
      && /function refreshMapDataForScenarioApply\([\s\S]*?const atlantropaWaterFeatureCount = getEffectiveAtlantropaFeatures\(\)\.water\.length;[\s\S]*?if \(atlantropaWaterFeatureCount > 0\) \{[\s\S]*?rebuildAuxiliaryRegionIndexes\(\);[\s\S]*?buildSecondarySpatialIndexes/.test(rendererSource)
