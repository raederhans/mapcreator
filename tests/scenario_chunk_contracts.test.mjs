...
test("exact-after-settle keeps scenario overlays on the contextScenario reuse path", () => {
  ...
  const contract = {
    ...
    exactAfterSettleSuccessInvalidatesPoliticalPass:
      /function invalidateExactAfterSettlePoliticalPass\(plan\) \{[\s\S]*?invalidateRenderPasses\("political", "exact-after-settle-political"\);[\s\S]*?plan\.politicalInvalidationReason = "exact-after-settle-political";[\s\S]*?plan\.politicalInvalidatedAt = politicalInvalidatedAt;/.test(exactSchedulerSource)
      && /function prepareExactAfterSettlePassesInSlices\(generation, plan\) \{[\s\S]*?if \(runtimeState\.renderPhase !== renderPhaseIdle\) \{[\s\S]*?abortInterruptedExactAfterSettleRefresh\(`\$\{passName\}-phase-interrupted`, generation\);[\s\S]*?return;[\s\S]*?if \(!isExactAfterSettleIdentityCurrent\(activeController\)\) \{[\s\S]*?abortInterruptedExactAfterSettleRefresh\(`\$\{passName\}-identity-mismatch`, generation\);[\s\S]*?return;[\s\S]*?if \(passName === "political"\) \{[\s\S]*?invalidateExactAfterSettlePoliticalPass\(plan\);[\s\S]*?getRenderPipelinePassesOwner\(\)\.prepareIdleRenderPassDefinition\(passName, drawFn, transform, timings, cache\);/.test(exactSchedulerSource)
    ...
    exactAfterSettleAbortsInterruptedBeforePaint:
      /function abortInterruptedExactAfterSettleRefresh\(reason = "interrupted", generation = null\) \{[\s\S]*?const shouldRearmExactRefresh = !!runtimeState\.deferExactAfterSettle;[\s\S]*?recordRenderPerfMetric\("settleExactRefreshAbortBeforePaint"[\s\S]*?resetExactAfterSettleController\(`abort-\$\{normalizedReason\}`, generation\);[\s\S]*?runtimeState\.deferExactAfterSettle = shouldRearmExactRefresh;[\s\S]*?runtimeState\.pendingExactPoliticalFastFrame = shouldRearmExactRefresh;[\s\S]*?invalidateRenderPasses\("political", "exact-after-settle-abort"\);[\s\S]*?if \(shouldRearmExactRefresh\) \{[\s\S]*?scheduleExactAfterSettleRefresh\(runtimeState\.adaptiveSettleProfile \|\| getAdaptiveSettleProfile\(\)\);[\s\S]*?\}[\s\S]*?requestRendererRender\("exact-after-settle-abort-recover", \{[\s\S]*?flush: false,[\s\S]*?if \(getContext\(\)\) render\(\);/.test(exactSchedulerSource)
    ...
    exactAfterSettleDefersContextPassesAfterCriticalPaint:
      ...
      && /function applyExactAfterSettleRefreshPlan[\s\S]*?resolveExactAfterSettleTargetPasses[\s\S]*?plan\.deferredExactTargetPasses = targetPassPlan\.deferredExactTargetPasses[\s\S]*?plan\.exactTargetPasses = targetPassPlan\.exactTargetPasses/.test(exactSchedulerSource)
...
test("post-edit visual override remains foreground after chunk promotion clears pending edit", () => {
  const rendererSource = readRepoFile("js", "core", "map_renderer.js");
  const harness = createRendererShellPolicyHarness(rendererSource);
  const primaryFallbackFeature = {
    id: "FR",
