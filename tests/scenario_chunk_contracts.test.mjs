...
test("exact-after-settle keeps scenario overlays on the contextScenario reuse path", () => {
  ...
  const contract = {
    ...
    exactAfterSettleSuccessInvalidatesPoliticalPass:
      ...
      && /function prepareExactAfterSettlePassesInSlices\(generation, plan\) \{[\s\S]*?let activePlan = plan;[\s\S]*?if \(runtimeState\.renderPhase !== renderPhaseIdle\) \{[\s\S]*?abortInterruptedExactAfterSettleRefresh\(`\$\{passName\}-phase-interrupted`, generation\);[\s\S]*?return;[\s\S]*?if \(!isExactAfterSettleIdentityCurrent\(activeController\)\) \{[\s\S]*?abortInterruptedExactAfterSettleRefresh\(`\$\{passName\}-identity-mismatch`, generation\);[\s\S]*?return;[\s\S]*?activePlan = invalidateExactAfterSettlePoliticalPass\(generation, activePlan\);[\s\S]*?getRenderPipelinePassesOwner\(\)\.prepareIdleRenderPassDefinition\(passName, drawFn, transform, timings, cache\);/.test(exactSchedulerSource)
