...
class ToolbarSplitBoundaryContractTest(unittest.TestCase):
    ...
    def test_transport_workbench_owner_moves_to_controller_module(self):
        ...
        self.assertIn("renderTransportWorkbenchPackSelect(family.id, context.activePackId);", shell_body)
        self.assertIn("export function getTransportWorkbenchPackOptionsSignature(packOptions) {", owner_content)
        self.assertIn("export function syncTransportWorkbenchPackSelectOptions({", owner_content)
        self.assertIn("syncTransportWorkbenchPackSelectOptions({", owner_content)
        self.assertIn("selectNode.dataset.packOptionsSignature !== nextSignature", owner_content)
        self.assertIn("selectNode.dataset.packOptionsSignature = nextSignature;", owner_content)
