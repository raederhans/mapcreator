...
DEFAULT_ENTRIES_OUTPUT_PATH = PROJECT_ROOT / "data" / "city_lights" / "historical_1930_entries.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--entries-output",
        default=str(DEFAULT_ENTRIES_OUTPUT_PATH),
        help="Output JSON entries asset path.",
    )
    parser.add_argument(
        "--exclusions-file",
...
def repo_relative_ref(path: Path) -> str:
    try:
        ...
        raise ValueError(f"Historical city lights input must live under repo root: {path}") from exc


def js_module_ref(from_module_path: Path, target_path: Path) -> str:
    relative_ref = Path(os.path.relpath(target_path, from_module_path.parent)).as_posix()
    if relative_ref.startswith("."):
        return relative_ref
    return f"./{relative_ref}"
...
import historical1930CityLightsEntriesPayload from "{entries_import_ref}" with {{ type: "json" }};
...
export const HISTORICAL_1930_CITY_LIGHTS_SOURCE = Object.freeze({format_object_literal({
    ...
    "sourceRefParts": source_ref.split("/", 1) if "/" in source_ref else [source_ref],
    "exclusionsRefParts": exclusions_ref.split("/", 1) if "/" in exclusions_ref else [exclusions_ref],
    "entriesRefParts": entries_ref.split("/", 1) if "/" in entries_ref else [entries_ref],
})});
...
def write_entries_asset(
    ...
    *,
    ...
    payload = {
        "schema_version": 1,
        "asset_key": "city_lights:historical_1930:entries",
        "family_id": "historical_1930",
        "stats": stats,
        "entries": entries,
    }
    entries_output_path.parent.mkdir(parents=True, exist_ok=True)
    entries_output_path.write_text(
...
def main() -> int:
    ...
    entries_output_path = Path(args.entries_output).expanduser().resolve()
    ...
    write_module(
        output_path,
        source_ref=repo_relative_ref(source_path),
        exclusions_ref=repo_relative_ref(exclusions_path),
        entries_ref=repo_relative_ref(entries_output_path),
        entries_import_ref=js_module_ref(output_path, entries_output_path),
        stats=stats,
    )
    print(
        f"Built historical city lights asset: {output_path} "
        f"and {entries_output_path} "
