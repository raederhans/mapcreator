...
function gitOutput(args, runner = spawnSync) {
...
export function resolveDefaultBaseSha({
  includeBranchHistory = false,
  runner = spawnSync,
} = {}) {
  const candidates = includeBranchHistory
    ? [
  ...
  for (const args of candidates) {
    const value = gitOutput(args, runner);
...
export function buildChangeDossier({
  ...
  return {
    ...
    gitSha: gitSha || gitOutput(["rev-parse", "HEAD"], runner) || "unknown",
    baseSha: baseSha || resolveDefaultBaseSha({ includeBranchHistory, runner }) || "unknown",
