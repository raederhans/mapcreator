...
const REQUIRED_ARTIFACT_PIPELINE_METHODS = Object.freeze([
...
function assertArtifactPipeline(artifactPipeline) {
  if (!artifactPipeline || typeof artifactPipeline !== "object") {
    throw new TypeError("createExportWorkbenchController requires artifactPipeline to be an object.");
  }
  REQUIRED_ARTIFACT_PIPELINE_METHODS.forEach((methodName) => {
    assertRequiredCallableDependency(artifactPipeline[methodName], `artifactPipeline.${methodName}`);
  });
}
      ...
      exportWorkbenchBakeVisibleBtn.addEventListener("click", async () => {
        ...
        try {
          for (const layerId of getBakePackLayerIds(exportUi)) {
            await bakeLayer(layerId, exportUi);
          }
          renderExportWorkbenchUi(true);
